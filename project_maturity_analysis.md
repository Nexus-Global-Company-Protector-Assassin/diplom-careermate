...
- [x] **1. Нет реальной multi-user изоляции**
Весь код использует `profile.findFirst()` — буквально первый профиль из базы. Если зарегистрируются два пользователя — все их данные перемешаются. JWT есть, но `userId` из токена почти нигде не используется для фильтрации. Добавь регистрацию и авторизацию
