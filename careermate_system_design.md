...
### 0. Backend Infrastructure (NestJS 10.x)
...
├── nest-cli.json                # Nest CLI конфигурация
├── package.json                 # Зависимости и скрипты
└── README.md
```

**Ключевые особенности:**
- **TypeScript 5.x** с ES2021 target и decorator support
- **NestJS 10.x** с модульной архитектурой
- **Prisma ORM** для работы с PostgreSQL
- **JWT аутентификация** с refresh token стратегией
- **Swagger/OpenAPI** документация на `/api/docs`
- **Валидация DTO** с class-validator
- **Rate limiting** через @nestjs/throttler
- **Логирование** через Winston с разными уровнями
- **Security headers** через helmet
- **Request compression** через compression middleware

**Зависимости:**
- @nestjs/core, common, platform-express
- @nestjs/config (управление env переменными)
- @nestjs/jwt, passport (аутентификация)
...
- @sendgrid/mail (email сервис)
- aws-sdk (S3 storage)
- helmet, compression (security и производительность)
- winston (логирование)

**Конфигурация:**
- **Порт:** 3001 (настраивается через APP_PORT)
- **JWT секреты:** через env переменные
- **Rate limiting:** 100 req/min для авторизованных, 10 req/min для анонимных
