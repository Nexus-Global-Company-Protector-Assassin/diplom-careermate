...
G_Legitimacy) Ghost Job Detection — оцени реальность вакансии:
  - ${freshnessNote}
  - Качество описания: есть ли конкретные технологии, команда, требования?
  - Противоречия в JD (junior-тайтл + 5 лет опыта)?
  - Зарплата указана? (позитивный сигнал)
  Вердикт: "High Confidence", "Proceed with Caution" или "Suspicious".
        ...
        if (!apiKey) {
            ...
            return {
                ...
                "G_Legitimacy": {
                    ...
                    "signals": ["[Mock] Добавьте LLM_API_KEY для реального Ghost Job Detection"],
                    "explanation": "Mock-режим: анализ недоступен без API ключа"
                },
                "archetype": archetype || "Unknown",
                "grade": mockGrade,
                "score": mockScore
